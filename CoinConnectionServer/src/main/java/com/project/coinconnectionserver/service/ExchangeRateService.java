...
import com.project.coinconnectionserver.controller.exchangeRate.ExchangeRateCrawler;
import com.project.coinconnectionserver.persistence.DTO.ExchangeRateDTO;
...
public class ExchangeRateService {
    ...
    public ExchangeRateDTO getRate() {
        ExchangeRateInfo exchangeRateInfo = exchangeRateInfoRepository.findByRateId(1)
                .stream()
                .findFirst()
                .orElseThrow(() -> new RuntimeException("Exchange rate not found"));
        return convertEntityToRateDTO(exchangeRateInfo);
    }

    private ExchangeRateDTO convertEntityToRateDTO(ExchangeRateInfo exchangeRateInfo) {
